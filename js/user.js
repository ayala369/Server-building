class User {
    ...
    signUp(ev) {
        ...
        fRequest.fOnload = function() { //כשהבקשה נטענת בדיקה אם ניתן לבצע את הבקשה
            if (fRequest.fResponse === "404") //אם התשובה היא שגיאה
            {
    ...
    logIn() {
        ...
        fRequest.fOnload = function() { //טעינת הבקשה
            if (fRequest.fResponse === "404") //שלא הכניס את הפרטים הנכונים אם התקבלה שגיאה
            {
...
class List {
    ...
    addList(ev) {
        ...
        fRequest.fOnload = function() { //בקשה נטענת
            if (fRequest.fResponse === "404") //קיימת רשימה עם שם זהה
            {
...
function buildCode() {
    const fRequest = new FXMLHttpRequest();
    fRequest.responseType = 'json';
    let url = "https://www.myList.com/api/user/buildCodeToLists"
    fRequest.fOpen('GET', url);
    fRequest.fOnload = function() {
        if (fRequest.fResponse === "404") {
            alert("Something went wrong...");
